...
# Bump version.
export CI_COMMIT_TAG=1.0.1  # <---- NOTE: UPDATE ME BEFORE RUNNING THE BELOW
...
aws s3 cp volume-autoscaler-${CI_COMMIT_TAG}.yaml s3://devops-nirvana/volume-autoscaler/volume-autoscaler-${CI_COMMIT_TAG}.yaml
...
PREVIOUS_RELEASE_TAG=$(cat README.md | grep -i "Current Release:" | awk '{ print $3 }')
sed -i "s/$PREVIOUS_RELEASE_TAG/$CI_COMMIT_TAG/g" README.md
git add .github/farley-publish-notes.md
